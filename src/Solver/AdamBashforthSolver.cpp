...
std::pair<std::vector<double>, std::vector<double>> AdamBashforthSolver::solve() const {
  ...
  for (int i = 0; i < std::min(this->n, 4); ++i) {
    ...
    double k2 = this->dt * this->f(t + 0.5 * this->dt, y + 0.5 * k1);
    double k3 = this->dt * this->f(t + 0.5 * this->dt, y + 0.5 * k2);
  ...
  for (int i = 4; i < this->n; ++i) {
    y = ys[i - 1] + this->dt * (55 * this->f(t, ys[i - 1]) - 59 * this->f(t - this->dt, ys[i - 2]) + 37 * this->f(t - 2 * this->dt, ys[i - 3]) - 9 * this->f(t - 3 * this->dt, ys[i - 4])) / 24;
    t += this->dt;
